...
export default function RecentActivity({ bookings }: RecentActivityProps) {
    if (!bookings || bookings.length === 0) {
        return (
            <GlassCard className="p-8 text-center text-slate-500 flex flex-col items-center justify-center h-full gap-3">
    ...
    return (
        <GlassCard className="flex flex-col h-full relative group p-0">
            {/* Header */}
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-white/60 backdrop-blur-md sticky top-0 z-20">
                <div className="flex items-center gap-3">
                    <span className="text-2xl">⚡</span>
                    <h3 className="text-lg font-black text-slate-900 tracking-tight">Recent Activity</h3>
                </div>
                <div className="flex items-center gap-2 px-2 py-1 bg-orange-50 text-orange-600 rounded-full border border-orange-100/50">
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50/50 backdrop-blur-sm relative z-20">
                <Link href="/admin/bookings" className="block w-full">
                    <button className="w-full py-3 text-sm font-bold text-slate-500 hover:text-slate-900 bg-white hover:bg-slate-50 border border-slate-200 hover:border-slate-300 rounded-xl transition-all shadow-sm hover:shadow flex items-center justify-center gap-2 group/btn">
                        View All Activity
